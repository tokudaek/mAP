...
for xml in ground_truth_files_list:
    #print(txt_file)
    file_id = xml.split(".xml",1)[0]
    file_id = os.path.basename(os.path.normpath(file_id))
    # check if there is a correspondent predicted objects file
    if not os.path.exists(args.preddir + '/' + file_id + ".xml"):
        error_msg = "Error. File not found: " + args.preddir + '/' + file_id + ".xml\n"
        error_msg += "(You can avoid this error message by running extra/intersect-gt-and-pred.py)"
        error(error_msg)

    #lines_list = file_lines_to_list(xml)
    ...
    for obj in objects:
        ...
        try:
            ...
            print(obj)
            error_msg = "Error: File " + xml + " in the wrong format.\n"
...
predicted_files_list = glob.glob(args.preddir + '/*.xml')
predicted_files_list.sort()

for class_index, class_name in enumerate(gt_classes):
    bounding_boxes = []
    for xml in predicted_files_list:
        #print(xml)
        ...
        if class_index == 0:
            if not os.path.exists(args.gnddir + '/' + file_id + ".xml"):
                error_msg = "Error. File not found: " + args.gnddir + '/' + file_id + ".xml\n"
                error_msg += "(You can avoid this error message by running extra/intersect-gt-and-pred.py)"
                error(error_msg)
        objects = parse_rec(xml)
        # create ground-truth dictionary
